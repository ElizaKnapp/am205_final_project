...
def load_kaggle_top_songs(csv_path: str) -> pd.DataFrame:
    ...
    """
    print("Loading Kaggle dataset from %s", csv_path)
    ...
    print("Loaded Kaggle dataset with %d rows after cleaning.", len(df))
    return df
...
def initialize_billboard_dataset(data_home: Optional[str] = None):
    ...
    try:
        ...
        print("Loaded Billboard dataset with %d tracks.", len(tracks))
    except Exception as exc:
        print(
...
def build_billboard_lookup(
    ...
    print("Billboard lookup built with %d unique (title, artist) keys.", len(lookup))
    return lookup
...
def select_songs_year_range(
    ...
    for year in range(start_year, end_year + 1):
        ...
        if df_y.empty:
            print("No Kaggle songs for year %d.", year)
            continue
...
def analyze_song_row(
    ...
    print("Analyzing song %s - %s (%d, decade %d).", artist, title, year, decade)
...
def run_song_level_analysis(
    ...
    """
    print("Starting song-level analysis for %d selected songs.", len(df_sel))
    ...
    if checkpoint_dir is not None:
        ...
        if resume and song_results_path.exists():
            ...
            print("No existing song-level checkpoint found or resume=False.")
    ...
    print("Song-level analysis produced %d total results.", len(df_results))
    return df_results
...
def aggregate_stationary_by_decade_from_tracks(
    ...
    """
    print("Starting decade-level aggregated Markov chain construction.")
...
def plot_tspec_vs_iters_loglog(df_results: pd.DataFrame) -> None:
    ...
    if df_results.empty:
        print("df_results is empty; skipping t_spec vs iters plot.")
        return
...
def aggregate_by_decade_from_song_results(df_results: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    if df_results.empty:
        print("df_results is empty; cannot aggregate by decade.")
        return pd.DataFrame()

    if "decade" not in df_results.columns:
        print("'decade' column missing in df_results; cannot aggregate.")
        return pd.DataFrame()
    ...
    print("Aggregated decade-level stats from song results:\n%s", df_decade)
    return df_decade
...
def aggregate_by_year_from_song_results(df_results: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    if df_results.empty:
        print("df_results is empty; cannot aggregate by year.")
        return pd.DataFrame()
    ...
    print("Aggregated year-level stats from song results:\n%s", df_year)
    return df_year
...
def plot_decade_trends(df_decade: pd.DataFrame) -> None:
    ...
    """
    if df_decade.empty:
        print("df_decade is empty; skipping decade trend plot.")
        return
...
def plot_year_trends(df_year: pd.DataFrame) -> None:
    ...
    """
    if df_year.empty:
        print("df_year is empty; skipping year trend plot.")
        return
...
def plot_histograms(df_results: pd.DataFrame) -> None:
    ...
    """
    if df_results.empty:
        print("df_results is empty; skipping histograms.")
        return
...
def plot_gap_vs_states(df_results: pd.DataFrame) -> None:
    ...
    """
    if df_results.empty:
        print("df_results is empty; skipping gap vs states plot.")
        return
...
def main():
    ...
    try:
        ...
        print("Saved selected songs to %s.", selected_songs_path)
    except Exception as exc:
        print("Failed to save selected songs checkpoint: %s", exc)
    ...
    if df_results.empty:
        print("No songs were successfully analyzed; skipping regression and plots.")
    else:
        # Save final song-level results
        try:
            df_results.to_csv(song_results_path_final, index=False)
            print("Saved final song-level results to %s.", song_results_path_final)
        except Exception as exc:
            print("Failed to save final song-level results: %s", exc)
        ...
        if not df_year.empty:
            try:
                ...
                print("Saved year-level averages to %s.", year_song_agg_path)
            except Exception as exc:
                print(
    ...
    if df_decade_chain.empty:
        print("Decade-level Markov-chain stats are empty.")
    else:
